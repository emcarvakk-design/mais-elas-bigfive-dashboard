...
function InfoCard({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="p-4 rounded-xl bg-gray-50 border border-gray-200">
      ...
      <p className="text-sm font-medium text-gray-900" style={color ? { color } : {}}>
        {value || '—'}
      </p>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4 shadow-sm">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-xs text-gray-400">LQA Dashboard · Confidencial</p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-6">
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1">
              <div className="flex items-start justify-between flex-wrap gap-3">
                <div>
                  <h1 className="text-xl font-bold text-gray-900">{profile.respondente.nome}</h1>
                  <p className="text-sm text-gray-500">{profile.respondente.email}</p>
        ...
        <div>
          ...
          {activeTab === 'bigfive' && (
            <Card className="bg-white border-gray-200 p-6 shadow-sm">
              ...
              <div className="bg-indigo-50 rounded-xl p-5 border border-indigo-200 mb-6">
                <p className="text-xs text-indigo-600 mb-2 font-medium">Hipótese baseada no perfil LQA</p>
                <p className="text-sm text-gray-800 leading-relaxed">
                  {cl?.perfil_codigo && (
                    <span>
                      O perfil <strong className="text-indigo-700">{cl.perfil_codigo} — {cl.perfil_nome}</strong> está
          ...
          {/* ABA 2 — BLANCHARD */}
          {activeTab === 'blanchard' && (
            <Card className="bg-white border-gray-200 p-6 space-y-6 shadow-sm">
              <div>
                  ...
                  <>
                        ...
                        return (
                          <div
                            ...
                            className={`p-4 rounded-xl border transition-all ${
                              isActive ? 'border-2' : 'border border-gray-200 opacity-50'
                            }`}
                            style={isActive ? {
                          ...
                          >
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-sm font-bold" style={isActive ? { color: info.color } : { color: '#9ca3af' }}>
                                {q}
                              </span>
                    ...
                    {cl._blanchard_biblioteca && (
                      <div className="mt-4 p-4 rounded-xl bg-indigo-50 border border-indigo-200">
                        <p className="text-xs text-indigo-600 font-medium mb-2">Padrão central (biblioteca LQA)</p>
                        <p className="text-sm text-gray-800">{cl._blanchard_biblioteca.padrao}</p>
                        <p className="text-xs text-gray-500 mt-2">{cl._blanchard_biblioteca.porque}</p>
                      </div>
                    )}
                  </>
                )}
                      ...
                      return (
                        <div key={nivel} className="flex items-center gap-3">
                          <div
                            ...
                            style={{
                              width,
                              backgroundColor: isDominante ? '#6366f1' : isGap ? '#fee2e2' : '#f3f4f6',
                              border: isDominante ? '2px solid #6366f1' : isGap ? '2px solid #ef4444' : '1px solid #e5e7eb',
                              color: isDominante ? '#fff' : isGap ? '#dc2626' : '#6b7280',
                            }}
                          >
                            {nivel}
                          </div>
              ...
              {cl?.meta_programas && (
                <>
                  ...
                  <div className="p-5 rounded-xl bg-indigo-50 border border-indigo-200">
                    <p className="text-xs text-indigo-600 font-medium mb-2">Padrão dominante</p>
                    <p className="text-sm font-semibold text-gray-900 mb-3">{cl.meta_programas.padrao_dominante}</p>
                    <p className="text-xs text-gray-500 font-medium mb-1">Implicação operacional</p>
                    <p className="text-sm text-gray-700 leading-relaxed">{cl.meta_programas.implicacao_operacional}</p>
                  </div>
                </>
              )}
            </Card>
              ...
              {cl?.fpc && (
                <div className="space-y-4">
                  <div className="p-5 rounded-xl bg-gray-50 border border-gray-200">
                    <p className="text-xs text-gray-500 mb-1">Dimensão dominante</p>
                    <p className="text-lg font-bold text-gray-900">{cl.fpc.dimensao_dominante}</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <InfoCard label="Padrão dominante" value={cl.fpc.padrao_dominante} />
                    <InfoCard label="Força central" value={cl.fpc.forca_central} color="#059669" />
                    <div className="md:col-span-2">
                      <div className="p-4 rounded-xl bg-red-50 border border-red-200">
                        ...
                        <p className="text-sm text-gray-700">{cl.fpc.risco_operacional}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === 'consolidado' && (
            <Card className="bg-white border-gray-200 p-6 space-y-6 shadow-sm">
              ...
              {cl?.lqa_consolidado && (
                <div className="space-y-4">
                  {/* Perfil resumo */}
                  <div className="p-5 rounded-xl border-2 bg-indigo-50" style={{ borderColor: `${mgColor}50` }}>
                    <div className="flex items-center gap-3 mb-3">
                      <div
                        className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm"
                        style={{ backgroundColor: `${mgColor}20`, color: mgColor }}
                      >
                        {cl.perfil_codigo}
                      </div>
                      <div>
                        <p className="font-semibold text-gray-900">{cl.perfil_nome}</p>
                        <p className="text-xs text-gray-500">{cl.macrogrupo_nome}</p>
                      </div>
                    </div>
                    {cl.perfil_secundario && (
                      <p className="text-xs text-gray-400">
                        Perfil secundário: <span className="text-gray-600">{cl.perfil_secundario}</span>
                      </p>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="p-4 rounded-xl bg-red-50 border border-red-200">
                      ...
                      <p className="text-sm text-gray-700 leading-relaxed">{cl.lqa_consolidado.gap_prioritario}</p>
                    </div>
                    <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200">
                      ...
                      <p className="text-sm text-gray-700 leading-relaxed">{cl.lqa_consolidado.forca_dominante}</p>
                    </div>
                    <div className="p-4 rounded-xl bg-amber-50 border border-amber-200">
                      ...
                      <p className="text-sm text-gray-700 leading-relaxed">{cl.lqa_consolidado.risco_ativado}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 rounded-xl bg-gray-50 border border-gray-200">
                      <p className="text-xs text-indigo-600 font-medium mb-2">Ação inicial sugerida</p>
                      <p className="text-sm text-gray-700 leading-relaxed">{cl.lqa_consolidado.acao_inicial}</p>
                    </div>
                    <div className="p-4 rounded-xl bg-gray-50 border border-gray-200">
                      <p className="text-xs text-gray-500 font-medium mb-2">Evidência esperada de evolução</p>
                      <p className="text-sm text-gray-700 leading-relaxed">{cl.lqa_consolidado.evidencia_esperada}</p>
                    </div>
                  </div>
                  ...
                  {cl._biblioteca?.trilha_especifica && (
                    <div className="p-5 rounded-xl bg-gray-50 border border-gray-200">
                      <p className="text-xs text-gray-500 font-medium mb-3">Trilha específica de desenvolvimento</p>
